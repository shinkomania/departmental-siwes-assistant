...
class Config:
    ...
    # If DATABASE_URL was set to relative in .env, normalize it
    if SQLALCHEMY_DATABASE_URI.startswith('sqlite:///instance/'):
        SQLALCHEMY_DATABASE_URI = f"sqlite:///{_db_path}"
    SQLALCHEMY_TRACK_MODIFICATIONS = False
    
    # Placement Web Search API Configuration
    # Supports extensible providers (e.g. SerpAPI, Tavily, Google Custom Search)
    SEARCH_API_KEY = os.environ.get('SEARCH_API_KEY', None)
    SEARCH_PROVIDER = os.environ.get('SEARCH_PROVIDER', 'none')
    
    # Admin Credentials (for initial development setup)
    ADMIN_USERNAME = os.environ.get('ADMIN_USERNAME', 'admin')
    ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD')
    
    # Application Metadata
    APP_NAME = "Departmental SIWES Assistant"
    APP_SUBTITLE = "Your guide to SIWES information, placement opportunities and industrial training support."
